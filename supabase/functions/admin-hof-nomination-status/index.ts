  ...
  return `
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
      ...
      <div style="background: #f8f9fa; padding: 30px; border: 1px solid #e0e0e0; border-top: none;">
        ...
        <p>Thank you for your Hall of Fame nomination. After careful review by our committee, we regret to inform you that the nomination was not approved at this time.</p>
        ...
        <p>We appreciate your commitment to recognizing outstanding individuals in pupil transportation. You are welcome to submit a new nomination in the future that addresses the concerns noted above.</p>
...
Deno.serve(async (req) => {
  ...
  try {
    console.log(`Processing ${req.method} request for HOF nomination status`);
    ...
    // Check if nomination is already finalized (approved or rejected)
    // Also fetch full nomination data for email
    const { data: existingNomination, error: fetchError } = await supabaseAdmin
      .from('hall_of_fame_nominations')
      .select('status, supervisor_first_name, supervisor_last_name, supervisor_email, nominee_first_name, nominee_last_name, district, grand_division, region')
      .eq('id', body.id)
      .single();
    ...
    // For approval, require admin verification flag
    ...
    // Build update data - include verification audit info when approving
    ...
    if (body.status === 'approved') {
      ...
      // Clear both if status is pending
      updateData.admin_verified_by = null;
      updateData.admin_verified_at = null;
    ...
    if (body.status === 'approved' || body.status === 'rejected') {
      const nominatorName = `${existingNomination.supervisor_first_name} ${existingNomination.supervisor_last_name}`;
      const nomineeName = `${existingNomination.nominee_first_name} ${existingNomination.nominee_last_name}`;
      const grandDivision = existingNomination.grand_division || `${existingNomination.region} Tennessee`;
      ...
      try {
        ...
        if (body.status === 'approved') {
          emailSubject = `🏆 Hall of Fame Nomination Approved - ${nomineeName}`;
          emailHtml = generateApprovalEmail({
            nominatorName,
            nomineeName,
            district: existingNomination.district,
            grandDivision
          });
        } else {
        ...
        const emailResult = await sendEmail({
          to: existingNomination.supervisor_email,
          subject: emailSubject,
          html: emailHtml
        });
        
        if (emailResult.success) {
          console.log(`✅ Status notification email sent to ${existingNomination.supervisor_email}`);
        } else {
    ...
    try {
      await supabaseAdmin.from('admin_logs').insert([{
        user_id: user.id,
        action: `update_hof_nomination_status_${body.status}`,
        outcome: 'success',
    ...
    console.log("HOF nomination status updated successfully");
    return new Response(
      JSON.stringify({ 
        success: true,
    ...
    console.error("Error in admin-hof-nomination-status function:", error);
    return new Response(
      JSON.stringify({
        success: false,
