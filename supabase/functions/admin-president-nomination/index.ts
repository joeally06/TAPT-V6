...
function generateApprovalEmail(data: {
  ...
  district: string;
  region: string;
  recipientRole: 'nominator' | 'nominee';
}): string {
  const isNominee = data.recipientRole === 'nominee';
  const introText = isNominee
    ? `Congratulations! We are pleased to inform you that your nomination for TAPT President has been approved by the nomination committee.`
    : `We are pleased to inform you that the TAPT President nomination you submitted for <strong>${data.nomineeName}</strong> has been approved by the nomination committee.`;
  ...
  return `
    ...
    <head>
      ...
      <title>TAPT President Nomination Approved</title>
    </head>
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
      <div style="background: linear-gradient(135deg, #059669 0%, #10b981 100%); padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
        <h1 style="color: white; margin: 0; font-size: 24px;">🏛️ Congratulations!</h1>
        <p style="color: #e0e0e0; margin: 10px 0 0 0; font-size: 16px;">President Nomination Approved</p>
      </div>
      ...
      <div style="background: #f8f9fa; padding: 30px; border: 1px solid #e0e0e0; border-top: none;">
        ...
        <ol style="padding-left: 20px;">
          <li style="margin-bottom: 10px;">The nominee will be placed on the ballot for the TAPT President election.</li>
...
function generateRejectionEmail(data: {
  ...
  const isNominee = data.recipientRole === 'nominee';
  const introText = isNominee
    ? `Thank you for your interest in serving as TAPT President. After careful review by our committee, we regret to inform you that the nomination was not approved at this time.`
    : `Thank you for your TAPT President nomination. After careful review by our committee, we regret to inform you that the nomination was not approved at this time.`;
  ...
  return `
    ...
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>TAPT President Nomination Status Update</title>
    </head>
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
      <div style="background: linear-gradient(135deg, #7c3aed 0%, #a855f7 100%); padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
        <h1 style="color: white; margin: 0; font-size: 24px;">🏛️ TAPT President</h1>
      ...
      <div style="background: #7c3aed; padding: 20px; text-align: center; border-radius: 0 0 10px 10px;">
        ...
        <p style="color: #e0e0e0; margin: 5px 0 0 0; font-size: 12px;">
          <a href="https://tapt.org" style="color: #c4b5fd;">www.tapt.org</a>
        </p>
      </div>
    </body>
    </html>
...
// Send notification emails to both nominator and nominee
async function sendStatusEmails(nomination: any, status: string, rejectionReason?: string) {
  ...
  const emailResults = { nominator: false, nominee: false };

  const commonData = {
    nomineeName,
    district: nomination.nominee_school_district,
    region: nomination.nominee_region,
  };
  ...
  try {
    ...
    if (status === 'approved') {
      emailSubject = `✓ TAPT President Nomination Approved - ${nomineeName}`;
      emailHtml = generateApprovalEmail({ ...commonData, recipientName: nominatorName, recipientRole: 'nominator' });
    } else {
      emailSubject = `TAPT President Nomination Status Update - ${nomineeName}`;
  ...
  try {
    ...
    if (status === 'approved') {
      emailSubject = `✓ Your TAPT President Nomination Has Been Approved`;
      emailHtml = generateApprovalEmail({ ...commonData, recipientName: nomineeName, recipientRole: 'nominee' });
    } else {
      emailSubject = `TAPT President Nomination Status Update`;
      emailHtml = generateRejectionEmail({ ...commonData, recipientName: nomineeName, rejectionReason: rejectionReason || '', recipientRole: 'nominee' });
    }

    const result = await sendEmail({ to: nomination.nominee_email, subject: emailSubject, html: emailHtml });
    emailResults.nominee = result.success;
    if (result.success) {
      console.log(`✅ Status email sent to nominee: ${nomination.nominee_email}`);
    } else {
      console.error(`⚠️ Failed to send email to nominee:`, result.error);
    }
  } catch (err) {
    console.error('⚠️ Error sending nominee email:', err);
  }
  ...
  try {
    ...
    if (req.method === 'DELETE') {
      ...
      try {
        await supabaseAdmin.from('admin_logs').insert([{
          user_id: user.id,
          action: 'delete_president_nomination',
          outcome: 'success',
    ...
    if (req.method === 'POST') {
      ...
      const { data: existing, error: fetchError } = await supabaseAdmin
        .from('president_nominations')
        .select('status, nominator_first_name, nominator_last_name, nominator_email, nominee_first_name, nominee_last_name, nominee_email, nominee_school_district, nominee_region')
        .eq('id', body.id)
        .single();
      ...
      let emailResults = { nominator: false, nominee: false };
      ...
      try {
        await supabaseAdmin.from('admin_logs').insert([{
          user_id: user.id,
          action: `update_president_nomination_status_${body.status}`,
          outcome: 'success',
    ...
    if (req.method === 'PUT') {
      ...
      const { data: nomination, error: fetchError } = await supabaseAdmin
        .from('president_nominations')
        .select('status, rejection_reason, nominator_first_name, nominator_last_name, nominator_email, nominee_first_name, nominee_last_name, nominee_email, nominee_school_district, nominee_region')
        .eq('id', body.id)
        .single();
      ...
      try {
        await supabaseAdmin.from('admin_logs').insert([{
          user_id: user.id,
          action: 'resend_president_nomination_emails',
          outcome: 'success',
    ...
    console.error('Admin President Nomination error:', error);
